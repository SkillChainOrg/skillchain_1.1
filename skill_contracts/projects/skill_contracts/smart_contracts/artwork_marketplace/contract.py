from algopy import ARC4Contract, Account, BoxMap, Bytes, Global, GlobalState, Txn, UInt64, gtxn
from algopy.arc4 import Address, Bool, String, UInt64 as ARC4UInt64, abimethod
...
class ArtworkMarketplace(ARC4Contract):
    ...
    def __init__(self) -> None:
        ...
        self.artwork_owner = BoxMap(Bytes, Bytes, key_prefix=b"owner:")
        self.artwork_price = BoxMap(Bytes, UInt64, key_prefix=b"price:")
        self.artwork_creator_did = BoxMap(Bytes, Bytes, key_prefix=b"creator:")
    ...
    def register_artwork(
        ...
        key = artwork_id.bytes
        existing_owner = self.artwork_owner.get(key, default=Bytes(b""))
    ...
    def get_owner(self, artwork_id: String) -> Address:
        key = artwork_id.bytes
        owner = self.artwork_owner.get(key, default=Bytes(b""))
    ...
    def get_price(self, artwork_id: String) -> ARC4UInt64:
        ...
        owner = self.artwork_owner.get(key, default=Bytes(b""))
        assert owner != Bytes(b""), "Artwork not found"
        price = self.artwork_price.get(key, default=UInt64(0))
        return ARC4UInt64(price)

    @abimethod
    def acquire(self, artwork_id: String) -> Bool:
        key = artwork_id.bytes
        current_owner = self.artwork_owner.get(key, default=Bytes(b""))
        ...
        # The acquire app call is only valid when it is atomically grouped with
        # a matching payment that funds the configured treasury/admin wallet.
        assert payment.sender == Txn.sender, "Payment sender mismatch"
        assert payment.receiver == treasury_account or payment.receiver == admin_account, "Bad payment receiver"
