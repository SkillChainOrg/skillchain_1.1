...
api.interceptors.request.use((config) => {
  ...
  if (
    (apiKey && config.url?.startsWith('/institution')) ||
    config.url === '/issue' ||
    config.url === '/issue/batch' ||
    config.url?.startsWith('/batch/')
  ) {
...
export const issueBatch = (formData) =>
  api.post('/issue/batch', formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
  });
...
export const addArtwork = (formData) =>
  api.post('/add-artwork', formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
  });
export const getArtisan = (did) => api.get(`/artisan/${did}`);

export const acquireArtwork = ({
  artwork_id,
  collector_name,
  collector_email,
  wallet_address,
